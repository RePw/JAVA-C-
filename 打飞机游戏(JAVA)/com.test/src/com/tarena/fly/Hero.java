...
public class Hero extends FlyingObject {
	...
	public Bullet[] shoot() {
		...
		int yStep = 20; // 步
		if (doubleFire > 0 && HeroIndex != 1) { // 双倍火力
			Bullet[] bullets = new Bullet[2];
			bullets[0] = new Bullet(x + xStep, y - yStep); // y-yStep(子弹距飞机的位置)
			bullets[1] = new Bullet(x + 3 * xStep, y - yStep);
			return bullets;
		} else if (doubleFire > 0 && HeroIndex == 1) {
			Bullet[] bullets = new Bullet[3];
			bullets[0] = new Bullet(x+30, y-yStep,ShootGame.bullet2);
			bullets[1] = new Bullet(x+2*xStep, y,ShootGame.bullet2);
			bullets[2] = new Bullet(x+4*xStep-30, y-yStep,ShootGame.bullet2);
			return bullets;
		} else { // 单倍火力
	...
	public boolean hit(FlyingObject other) {
		...
		return herox > x1 + 70 && herox < x2 - 70 && heroy > y1 + 70 && heroy < y2 - 70; // 区间范围内为撞上了
	}
